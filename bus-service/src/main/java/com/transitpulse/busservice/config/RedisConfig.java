...
public class RedisConfig {
    ...
    @Bean
    public RedisTemplate<String, BusLocationEvent> liveBusRedisTemplate(
    ...
    @Bean
    public RedisTemplate<String, BusEtaEvent> etaRedisTemplate(
            RedisConnectionFactory connectionFactory,
            ObjectMapper redisObjectMapper) {

        RedisTemplate<String, BusEtaEvent> template = new RedisTemplate<>();
        template.setConnectionFactory(connectionFactory);
        template.setKeySerializer(new StringRedisSerializer());

        Jackson2JsonRedisSerializer<BusEtaEvent> serializer =
                new Jackson2JsonRedisSerializer<>(redisObjectMapper, BusEtaEvent.class);
        template.setValueSerializer(serializer);
        template.setHashValueSerializer(serializer);
